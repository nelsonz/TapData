...
{
  #ifdef MIFAREDEBUG
  Serial.print("Trying to write 16 bytes to block ");
  Serial.println(blockNumber);
  #endif
...
/**************************************************************************/
/*! 
    Tries to write an entire 4-byte page at the specified page
    address.
...
{
  #ifdef MIFAREDEBUG
  Serial.print("Trying to write 4 bytes to page ");
  Serial.println(page);
  #endif
  
  if (page < 4 || page > 15)
  {
    Serial.println("Invalid page address! Must be between 4..15");
    return 0;
  }
  ...
  pn532_packetbuffer[0] = PN532_COMMAND_INDATAEXCHANGE;
  pn532_packetbuffer[1] = 1;                      /* Card number */
  pn532_packetbuffer[2] = MIFARE_CMD_WRITE;       /* Mifare Write command = 0xA2 */
  pn532_packetbuffer[3] = page;                   /* Block Number (4..15 for 1K) */
